...
(`Coordinates`, `Tile`, `TileMap`, `MenuModel`, the letterbox math and the
hand-rolled `Linked_list`) and is wired into CTest.

### Map editor

A Dear ImGui map editor (`codemon_editor`) is built alongside the game (it
fetches Dear ImGui + ImGui-SFML at configure time, so the first configure needs
network access). Disable it with `-DCODEMON_BUILD_EDITOR=OFF`.

```sh
./build/codemon_editor
```

Paint tiles from the palette onto the grid (left click paints, right click
erases to grass), tick "Place player start" to move the spawn, resize the map,
and Save/Reload in the same CSV format the game loads. Maps live in
`codemon/maps/` and use the original tileset generated by
`tools/gen_overworld_tileset.py`; `tools/gen_region.py` authors the starter
region. All artwork is original to this project.
